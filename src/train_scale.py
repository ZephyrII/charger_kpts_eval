...
class ChargerDataset(utils.Dataset):
    ...
    def load_charger(self, dataset_dir, subset, scale):
    ...
    def load_kp(self, image_id, num_points):
        ...
        for object in root.findall('object'):
            ...
            for i in range(num_points):  # TODO: remove
                ...
                point_center = (int(np.round(point_center[0]*self.scale)/self.scale), int(np.round(point_center[1]*self.scale)/self.scale))
...
def train(model):
    ...
    dataset_train.load_charger(args.dataset, "train", float(args.scale))
    dataset_train.prepare()

    # Validation dataset
    dataset_val = ChargerDataset()
    dataset_val.load_charger(args.dataset, "val", float(args.scale))
    dataset_val.prepare()
