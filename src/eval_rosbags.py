...
for path in tqdm(paths, "Dirs"):
        ...
        for filename in tqdm(files, "Files", leave=False):
                if filename.endswith(".bag"):
                        play_command = "rosbag play --topics /dgps_ublox/dgps_base/fix /dgps_ublox/dgps_rover/navrelposned  /processed_can_data /blackfly/camera/image_color/compressed -r 0.06 -s 0 " +os.path.join(path, filename)
